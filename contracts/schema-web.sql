-- =============================================================================
-- 服务端建库 DDL · PostgreSQL 16 方言（server/web）
--
-- 权威性：本文件与 schema-d1.sql 语义完全一致，差异仅在方言
--         （见 server-plan.md §8.3 与本目录 README.md 的类型映射表）。
--         两文件的表/列集合一致性由 scripts/verify-schema.mjs 断言。
--
-- 约定（server-plan.md §5.1）：
--   - 命名 snake_case；主键 TEXT（uuid 或语义 id）
--   - 时间戳统一 epoch 毫秒（PG: BIGINT / D1: INTEGER）
--   - 布尔用 0/1 整数（PG: SMALLINT / D1: INTEGER），避免方言差异
--   - 状态一律 TEXT + CHECK 约束
--   - JSON 一律存 TEXT 字符串（应用层解析，不用 jsonb，保证两库迁移等价）
--   - 不存储分数明细、证明材料、任何私钥（§3.2 红线）
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS admin_user (
    ...
    -- web 架构为 scrypt，cloudflare 架构为 PBKDF2-SHA256；两库数据不互通（§7.1）
...
CREATE TABLE IF NOT EXISTS license (
    ...
    renewed_at  BIGINT,
    renew_count SMALLINT NOT NULL DEFAULT 0 CHECK (renew_count >= 0),
    created_at  BIGINT   NOT NULL,
    updated_at  BIGINT   NOT NULL
);
...
CREATE TABLE IF NOT EXISTS apply_status (
    ...
    latest_revision   SMALLINT CHECK (latest_revision IS NULL OR latest_revision >= 1),
    -- 管理端导入时上报的版本（与 latest_revision 比对提示索要最新版）
    imported_revision SMALLINT CHECK (imported_revision IS NULL OR imported_revision >= 1),
    imported_at       BIGINT,
    created_at        BIGINT   NOT NULL,
    updated_at        BIGINT   NOT NULL,
